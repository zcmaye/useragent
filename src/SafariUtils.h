...
class SafariUtils {
private:
	inline static const char* webKitToSafariVersion[][15]{
		{"48", "0.8"},
		{"73", "0.9"},
		{"85", "1.0"},
		{"85.8.5", "1.0.3"},
...
//		{"", "9.0"},
	...
	std::map<std::string, Version> m_safariVersions;
public:	
	static SafariUtils* instance()
	{
		static SafariUtils* su;
		if (!su)
		{
			su = new SafariUtils;
			for (auto& pair : webKitToSafariVersion) {
				...
				su->m_safariVersions.insert({ webKitVersion, version });
			}
		}
		return su;
	}

	const std::map<std::string, Version>& getWebKitToSafariVersion() {
		return m_safariVersions;
	}
};
